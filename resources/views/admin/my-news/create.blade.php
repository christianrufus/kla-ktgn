<x-app-layout>
    ...
    <div class="pl-4 py-12">
        <form id="ckeditor-form" action="{{ route('upload.image') }}" method="post" enctype="multipart/form-data"
            style="display: none;">
            @csrf
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <a href="{{ route('admin.my-news.index') }}"
                    ...
                    <form id="createForm" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="created_at" value="{{ now()->format('Y-m-d\TH:i') }}"
                                min="{{ now()->subDays(2)->format('Y-m-d\TH:i') }}"
                                max="{{ now()->addDays()->format('Y-m-d\TH:i') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                required>
                            ...
                            <div id="uploadProgress" class="mt-3 hidden">
                                <div class="flex justify-between mb-1">
                                    ...
                                    <div class="flex space-x-2">
                                        <p class="text-xs text-gray-700"><span id="uploadedSize">0</span>/<span
                                                id="totalSize">0</span> MB</p>
                                        <p class="text-xs text-gray-700"><span id="uploadSpeed">0</span> KB/s</p>
                                    </div>
                                </div>
                                <div class="w-full bg-gray-200 rounded-full h-2.5">
                                    <div id="progressBar" class="bg-blue-600 h-2.5 rounded-full" style="width: 0%">
                                    </div>
                                </div>
                            </div>
                    ...
                    axios.post('/api/news', formData, {
                        ...
                        .then(response => {
                            if (response.data.success) {
                                ...
                                setTimeout(() => {
                                    window.location.href = '{{ route("admin.my-news.index") }}';
                                }, 1000);
                            }
                        })
                        .catch(error => {
